...
root = tk.Tk()
root.title("Мнемосхема + Arduino")
canvas = tk.Canvas(root, width=1150, height=600, bg="white")
canvas.pack()

# Индикатор статуса Arduino
arduino_status_label = tk.Label(root, text="Arduino: not connected", bg="red", fg="white")
arduino_status_label.place(x=250, y=0)
...
# 8 кнопок Arduino -> 8 сегментов (по битам от младшего к старшему)
SEGMENT_ORDER = [
    ("M1", "pastM1"),  # бит 0
    ("M1", "M8"),      # бит 1
    ("M8", "H1"),      # бит 2
...
# занятость сегментов поездом (0 - занят, 1 - свободен)
seg_occ_train = {
    ("M1", "pastM1"): 1,
    ("M1", "M8"): 1,
    ("M8", "H1"): 1,
...
def update_all_occupancy():
    ...
    for (a, b), seg_id in segment_ids.items():
        occ_train = (seg_occ_train.get((a, b), 1) == 0 or
                     seg_occ_train.get((b, a), 1) == 0)

        if occ_train:
            paint_segment((a, b), "red")
        elif (a, b) in occupied_segments or (b, a) in occupied_segments:
    ...
    for diag_name, lines in diag_ids.items():
        ...
        elif diag_name in occupied_diagonals:
...
button.place(x=60, y=1)

# тестовые кнопки (для отладки без Arduino)
def do():
...
def bytes_to_bits(data: bytes, bits_needed: int) -> list[int]:
    """
    Превращает массив байтов в плоский список битов [0,1,...]
    Берём от младшего бита к старшему (LSB first):
    бит 0 -> первый элемент списка.
    """
    bits: list[int] = []
...
def apply_bits_to_segments(bits: list[int]):
    ...
    # первый раз – просто инициализируем и сразу всё синхронизируем
