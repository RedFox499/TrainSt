...
class SignalManager():
    ...
    def hide_H_fake(self):
        for id in self.signal_ids["H_fake"]:
            canvas.itemconfigure(id, state="hidden")
        for id in self.signal_ids_simple["H_fake"]:
            canvas.itemconfigure(id, state="hidden")
...
class interface_manager:
    ...
    def check(self):
        print(SignalManage.signals_state["H_fake"], "H_fake")
        print(SignalManage.signals_state["H"], "H")
        ...
        try:
            while arduino.in_waiting > 0:
                ...
                if line:
                    ...
                    # === УБРАЛИ ХЕШТЕГ, РАСКОММЕНТИРОВАЛИ ТУТ ===
