...
def recalc_signals_from_active_routes() -> None:
    ...
    for rid, data in active_routes.items():
        ...
        for name in cfg:
            for colors in cfg[name]['lamps']:
                if colors in signals_state[name]["lamps"]:
                    signals_state[name]["lamps"][colors]["on"] = True
...
def update_signals_visual_v2() -> None:
    ...
    for name in signals_config.keys():
        ...
        ids = signal_ids[name]  # [item_id ламп]
        cfg_colors = signals_config[name]["colors"]

        st = signals_state.get(name)
        if not st:
            continue
        ...
        for idx, oid in enumerate(ids):
            ...
            # если такой лампы физически нет в конфиге — гасим
            if color not in lamps:
                fill = SIGNAL_OFF_COLOR
            else:
                lamp = lamps[color]
                is_on = lamp["on"]
